		...
		try {
			...
			mess = {
				...
				only: {
					...
					benned: 'por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Este é um recurso especial para o proprietário do bot ❌',
					ownerB: '[❗] Este é um recurso especial para o proprietário do bot ❌',
					userB: `──「 LISTA 」──\nOlá\nDesculpe, você não está registrado ainda. Registre-se para utilizar o bot, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar wzz|14\n\n──「 Wzz BOT 」──`,
			...
			const ownerNumber = ["5511977782073@s.whatsapp.net"] // replace this with your number
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('pow cara, não fale essas coisas só pq vc é adm, isso pode ofender alguém e fazer a pessoa ficar depressivar KKKK')
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu numero oficial é esse ai, salvem contato la 🤙🏻',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					client.sendMessage(from, duaa, image, {caption: 'kkkjkkjk', quoted: mek})
					break
				case '3d':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5511977782073*╰──────*「 *WZZ* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO WZZ BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bodia zap'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'boa tarde zap'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'boa noite zap'})
					break
                case 'belle':                 
				...
				case 'boanoite':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bodia'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'setprefix':
					...
					break
                                case 'lolih':
                                        ...
                                        break
				case 'nsfwloli':
					if (!isNsfw) return reply('*ESSE COMANDO SÓ FUNCIONA NO PV AMIGO.*')
			...
			reply(mess.wait)
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('quem é voce random? 😂')
					...
					break
				case 'bc':
					if (!isOwner) return reply('quem é você random? 😂')
        ...
        case 'promover':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 para ativar, 0 para desativar, lerdão vc em KKKKK')
					}
					break
				case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
